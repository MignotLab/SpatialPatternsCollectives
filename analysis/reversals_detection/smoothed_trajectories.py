...
class SmoothedTrajectories:
    ...
    def __init__(self, par, df, end_min_size_smoothed_um, start_min_size_smoothed_um, step_min_size_smooth_um):
        ...
        # Allow to gradually increase the smooth size
        self.min_size_smoothed_list = np.arange(start_min_size_smoothed_um, end_min_size_smoothed_um, step_min_size_smooth_um) / self.par.scale
        ...
        self.id_frames = np.unique(self.df.loc[:, self.par.t_column].to_numpy())
        ...
        t = self.df.loc[:,self.par.t_column].to_numpy()
        x_all = self.df.loc[:,self.par.x_column].to_numpy()
        y_all = self.df.loc[:,self.par.y_column].to_numpy()
    ...
    def fill_trajectory_holes(self):
        ...
        id_array = self.df_detection.loc[:, self.par.track_id_column].to_numpy()
        frame_array = self.df_detection.loc[:, self.par.t_column].to_numpy()
        ...
        for i in tqdm(range(2, max_diff+1)):
            ...
            for j in range(1, i):
                ...
                coords_time_1 = self.df_detection.loc[cond_time_diff_i, columns_coord].to_numpy()
                coords_time_2 = self.df_detection.loc[np.roll(cond_time_diff_i, shift=1), columns_coord].to_numpy()
    ...
    #         self.t.append(self.df.loc[cond_traj, self.par.t_column].to_numpy())
    #         self.x.append(self.df.loc[cond_traj, self.par.x_column].to_numpy())
    #         self.y.append(self.df.loc[cond_traj, self.par.y_column].to_numpy())


    def coord_in_list(self):
        """
        Read the CSV file and extract trajectories into lists.
        The x, y, and t coordinates of each trajectory are stored 
        in self.x, self.y, and self.t respectively.
        """
        print("DATAFRAME X, Y and T TO LIST")
        grouped = self.df.groupby(self.par.track_id_column)

        for id_traj in tqdm(self.index_traj):
            group = grouped.get_group(id_traj)
            self.t.append(group[self.par.t_column].to_numpy())
            self.x.append(group[self.par.x_column].to_numpy())
            self.y.append(group[self.par.y_column].to_numpy())
    ...
    def smooth_small_displacement(self):
        ...
        for j in tqdm(range(len(self.x_s))):
            ...
            for min_size_smoothed in self.min_size_smoothed_list:
                for iteration in range(self.par.iteration):
                    ...
                    while i < len(x_s_tmp) - 2:
                        ...
                        if leng_tmp[i] < min_size_smoothed:
