...
class Matrix:
    ...
    def matrix_r_directional(self, xi, S, C):
        ...
        R_pos = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        R_A = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        R_neg = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        # A and D diagonal
        diag = np.zeros(self.par.ns, dtype=np.complex64)
        diag[:] = -1j*xi
    ...
    def matrix_p_directional(self, xi, S, C):
        ...
        """
        S_ind = int(S / self.par.ds)
        P_pos = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        P_A = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        P_neg = np.zeros((self.par.ns, self.par.ns), dtype=np.complex64)
        # A and D diagonal
        diag = np.zeros(self.par.ns, dtype=np.complex64)
        diag[:] = -1j*xi
        diag[:-1] -= self.par.delta
